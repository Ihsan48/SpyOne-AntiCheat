	...
	public function checkA(Event $event, Player $player): string
	{
		...
		if ($player->isOnGround() && $this->counter->getTick() > 5 && $this->jumpTickDifference > 10 && $this->jumpTickDifference < 20) {
			...
			if ($this->distance > $this->jumpTickDifference * $this->distancePerTickAllowed) {
				...
				if (PlayerUtil::movementSpeedInfluenced($player) || !BlockUtil::blockAbove($this->to, $player->getWorld())->getTypeId() === VanillaBlocks::AIR()->getTypeId() || PlayerUtil::recentlyHurt($player) || PlayerUtil::recentlyRespawned($player)) return "Movement speed influenced";
