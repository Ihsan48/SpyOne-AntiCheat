...
class BlockUtil
{
	...
	{
		return (!empty($vector1) && !empty($vector2)) ? sqrt(pow($vector1[0] - $vector2[0], 2) + pow(0, 2) + pow($vector1[2] - $vector2[2], 2)) : 0.0;
	}

	static function calculateDistanceWithY(array $vector1, array $vector2): float
	{
		return (!empty($vector1) && !empty($vector2)) ? sqrt(pow($vector1[0] - $vector2[0], 2) + pow($vector1[1] - $vector2[1], 2) + pow($vector1[2] - $vector2[2], 2)) : 0;
	}
